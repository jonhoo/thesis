...
ax.plot(data["achieved"], data["median"], '.-', color=common.colors['partial'], label="Noria, partial state")
...
ax.plot(data["achieved"], data["median"], '.-', color=common.colors['full'], ls='--', label="Noria, full state")
data = common.source['redis'].query('op == "all" & write_every == 1000 & until > 256 & metric == "sojourn"').sort_index().reset_index()
ax.plot(data["achieved"], data["median"], '.-', color=common.colors['redis'], label="Redis, 1 core")
ax.xaxis.set_major_formatter(common.kfmt)
mx = data.query("achieved >= 0.99 * target & median < 100")["achieved"].max()
print(mx, 16 * mx)
mx = 16 * mx
