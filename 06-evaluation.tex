...
too costly to use with current solutions. It presents partial state as a
solution to this problem that allows retaining the benefits of view
materialization at a fraction of the cost. This chapter evaluates the validity
of this assumption, and the efficacy of partial state as a solution by
evaluating the following questions:

\begin{enumerate}
  \item Why is view materialization desirable? (\S\ref{s:eval:why})
  \item Why is view materialization not feasible currently? (\S\ref{s:eval:why})
  \item Does partial state improve on the situation? (\S\ref{s:eval:why})
  \item Why use Noria over ad-hoc solutions? (\S\ref{s:eval:alts})
...
heavy. Table~\ref{t:lobsters-pages} gives an overview of how often each page is
...
also necessary: without them, Lobsters running against MySQL cannot keep up with
even a single request per second.
...
The remaining latency is primarily determined by the number of queries each page
issues, as each one requires a round-trip to Noria.
...
your requests must be computed on-demand. Figure~\ref{f:lobsters-mem-latency}
...
Figure~\ref{f:vote-migration} plots the cache hit rate seen by reads from the
...
Figure~\ref{f:vote-formula} plots $F(\alpha, N)/N$ after one second for
