...
pub(crate) async fn main() -> Result<(), Report> {
    let results = futures_util::future::join_all(vec![
        tokio::spawn(one(19, "skewed", 1).in_current_span()),
        tokio::spawn(one(19, "skewed", 6).in_current_span()),
    ])
    .await;
        ...
        let result: Result<(), Report> = try {
            ...
            while let Some(target) = targets.next() {
                ...
                async {
                    ...
                    let prefix = format!(
                        "netsoup.5000000a.{}t.{}r.{}c.{}",
                        target, write_every, nclients, distribution,
                    );
                    ...
                    let prime = crate::run_with_stderr(
                        &mut vote_client(cs[0], server, |cmd| {
                            cmd.arg("--warmup=0")
                                .arg("--runtime=0")
                                .arg("-d")
                                .arg(distribution)
                    ...
                    if prime.success() {
                        ...
                        let mut stderr = tokio::stream::StreamMap::new();
                        for (clienti, bench) in benches.iter_mut().enumerate() {
                            stderr.insert(
                                clienti,
                                tokio::io::BufReader::new(bench.stderr().take().unwrap()).lines(),
                            );
                        }
                        while let Some((clienti, line)) = stderr.next().await {
                            let line = line.wrap_err("failed to read client output")?;
                            tracing::trace!(client = clienti, message = &*line);
                        }
                        ...
                        for (clienti, bench) in benches.into_iter().enumerate() {
                            let status = bench.wait().await?;
                            if !status.success() {
                                tracing::warn!(client = clienti, "client failed");
                                targets.overloaded();
                            }
                        }
                    }
