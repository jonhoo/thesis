...
\begin{figure}
  ...
  would happen if the system has checked that there are indeed no rows for
  author C.}
  \label{f:indexing}
\end{figure}

Missing state manifests as missing entries in particular indices in the system.
Indexes over a given state are either all partial or none of them are. This may
seem strange based on how indices work in traditional relation databases.
...
because the system has checked upstream that there are indeed no stories written
by C.
...
chosen plan. There may be multiple possible candidates if there are multiple
...
one shown in Figure~\vref{f:key-prov} for the \texttt{StoriesWithVC} view from
...
In Listing~\ref{l:vote-src}, the system is asked to parameterize
...
The join in Listing~\ref{l:vote-src} uses a inner join ($\bowtie$), and the
...
The key provenance information from Figure~\vref{f:key-prov} gives the system
all the information it needs to set up these indexes: an index is needed on the
upquery key column on each state on the chosen upquery paths. In the case of the
view from Listing~\ref{l:vote-src}, an index is needed on
...
When the system evicts an entry from state in the middle of the dataflow, the
missing entry may cause an update to be discarded even though downstream entries
hold data that would grow stale without that update. For example, consider what
...
from non-partial state, the system would be unable to satisfy the invariant
